...
```bash
npm install
npm test          # 161 tests del motor, incluidos los adversariales
cd frontend && npx vitest run   # 116 más — 277 entre las dos suites
...
| Capa | Estado |
|---|---|
| Motor determinista + gate de Daubert | **Funciona**, 161 tests |
| Cobertura de tests entre las dos suites | **277 en verde** — 161 del motor (`npm test`) + 116 del frontend (`vitest run` en `frontend/`). Contados por los runners, no estimados — `node scripts/count-tests.mjs` vuelve a medir y falla si esta línea se desactualiza |
