...
registrada en su momento: **9/9 → 14/14 → 34/34 → 41/41 → 38/38 → 53/53 → 58/58 → 173/173** (la única
...
el port de VIGÍA a 115, y el propio gate de conteo de este documento a 173). Entre las dos suites los runners informan **289
tests pasando: 173 en el motor (`npm test`) y 116 en el frontend (`vitest run` en
...
| Suite raíz | 173/173 en verde | `npm test`, 2026-08-20 |
| Suite de frontend | 116/116 en verde | `vitest run` en `frontend/`, 2026-08-20 |
| **Las dos suites** | **289/289 en verde** | medido, no derivado — `node scripts/count-tests.mjs` |
...
runners informan **173** y **116**, que es lo que dice la tabla.
