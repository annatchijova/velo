...
registrada en su momento: **9/9 → 14/14 → 34/34 → 41/41 → 38/38 → 53/53 → 58/58 → 161/161** (la única
...
el port de VIGÍA a 115, y el propio gate de conteo de este documento a 161). Entre las dos suites los runners informan **277
tests pasando: 161 en el motor (`npm test`) y 116 en el frontend (`vitest run` en
...
| Suite raíz | 161/161 en verde | `npm test`, 2026-08-20 |
| Suite de frontend | 116/116 en verde | `vitest run` en `frontend/`, 2026-08-20 |
| **Las dos suites** | **277/277 en verde** | medido, no derivado — `node scripts/count-tests.mjs` |
...
runners informan **161** y **116**, que es lo que dice la tabla.
