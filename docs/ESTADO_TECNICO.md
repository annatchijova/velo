...
registrada en su momento: **9/9 → 14/14 → 34/34 → 41/41 → 38/38 → 53/53 → 58/58 → 170/170** (la única
...
el port de VIGÍA a 115, y el propio gate de conteo de este documento a 170). Entre las dos suites los runners informan **286
tests pasando: 170 en el motor (`npm test`) y 116 en el frontend (`vitest run` en
...
| Suite raíz | 170/170 en verde | `npm test`, 2026-08-20 |
| Suite de frontend | 116/116 en verde | `vitest run` en `frontend/`, 2026-08-20 |
| **Las dos suites** | **286/286 en verde** | medido, no derivado — `node scripts/count-tests.mjs` |
...
runners informan **170** y **116**, que es lo que dice la tabla.
