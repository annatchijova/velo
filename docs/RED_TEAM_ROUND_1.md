...
>
> **Contract status (added later the same day, 2026-08-07):** `contracts/velo.compact` — out of scope at audit time because it could not be compiled here — has since compiled successfully on a separate AVX2-capable machine (real prover/verifier keys generated, exit 0). See the F3 addendum and the corresponding line in "What was NOT verified" below for exactly what that does and doesn't establish.
...
| F3 | **High** | CONFIRMED | `bundle.ts` + docs | What gets committed on-chain (the fingerprint) **excluded the custody chain**: custody truncation was invisible to the planned anchor | **FIXED**, and widened post-audit — see addendum |
...
- **Addendum (2026-08-07, after the contract actually compiled): the shipped binding is wider than described above, and for a good reason.** Verified against the live `contracts/velo.compact`, not from memory: the deployed circuit does not hash a 3-element vector. It hashes **six**: `persistentHash<Vector<6, Bytes<32>>>([pad(32,"velo:attestation:v1"), fingerprint, tip, verdict as Field as Bytes<32>, corroborationCount as Field as Bytes<32>, salt])`. The contract's own comment names the reason, and it is a worse defect than the one F3 originally described: an earlier version took `verdict` as a free public circuit argument and `corroborationCount` as an *unbound* witness — neither was hashed into the commitment. A prover holding a legitimately sealed `NOISE` analysis could call `attest(commitment, MALICE)` with a fabricated `corroborationCount`, and the circuit's central claim ("the published verdict matches the sealed analysis") would be vacuous — the Daubert check would compare a self-declared number against itself. Binding `verdict` and `corroborationCount` into the commitment closes that: a given commitment can now only ever carry one verdict, full stop. This was found and fixed by the engine session after this audit, not flagged here originally — recorded here because the alternative was leaving this document asserting something the running contract no longer does.
- **Compiler verification (2026-08-07):** compiled successfully — `compact` 0.5.1 / `compactc` 0.31.1, on a machine with AVX2 (`zkir`'s proving-key backend needs it; without it the compiler dies with `SIGILL` after type-checking succeeds, which is a hardware limit, not a contract defect — reproduced on the original audit machine, a pre-AVX2 CPU). Exit 0, both circuits, real prover and verifier keys generated. That resolves, from the actual compiler rather than from the documented pattern: `persistentHash<Vector<N, Bytes<32>>>` compiles for `N=6`; `verdict as Field as Bytes<32>` is a legal cast for an enum (the change most likely to fail, since the pattern is documented for `Counter`, not enums); `disclose()` on already-public circuit arguments is accepted. Reproduce with `bash scripts/compile-contract.sh`.
...
- ~~`contracts/velo.compact` was not delivered at audit time and is still uncompiled — no Compact toolchain on the current machine.~~ **UPDATE (2026-08-07, later the same day): compiled successfully**, on a different machine with AVX2 (`compact` 0.5.1 / `compactc` 0.31.1, `bash scripts/compile-contract.sh`), real prover and verifier keys generated for both circuits, exit 0. This line stays struck through rather than deleted: it was true when written, and the audit's own scope note ("`contracts/velo.compact`... could not be audited") is accurate history, not a live status claim. What it means going forward: the authors' original two flagged points (`persistentHash` vs `persistentCommit`, whether `disclose()` is truly required) and the F3 addendum's cast question are no longer open — the compiler accepted all of them; see the addendum on F3 above for exactly what was checked. Still unverified: the circuit's *behavior* — that a valid proof actually gets accepted and an invalid one actually gets rejected — since that needs `zkir`'s proving/verifying keys to actually be exercised end-to-end (generated, not yet run against a test vector), and the deployed-contract integration into `attest_case` (currently a stub) has not been built.
