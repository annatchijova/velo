...
**9/9 → 14/14 → 34/34 → 41/41 → 38/38 → 53/53 → 58/58 → 161/161** (the one drop is the
...
this document's own count gate to 161).
Across both suites the runners report **277
passing tests: 161 in the engine (`npm test`) and 116 in the frontend
...
| Root suite | 161/161 green | `npm test`, 2026-08-20 |
| Frontend suite | 116/116 green | `vitest run` in `frontend/`, 2026-08-20 |
| **Both suites** | **277/277 green** | measured, not derived — `node scripts/count-tests.mjs` |
...
longer needed: both suites are run and counted. The runners report **161** and
