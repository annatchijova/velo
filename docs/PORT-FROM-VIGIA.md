...
| #5 trust_fusion | DEFERRED | VELO cases carry no per-artifact evidence age; porting the decay table now would be dead code. Revisit when validation timestamps exist. |
...
| #10-#15 narrative producers | NOT PORTED | Phase 4 remains open; the non-sealable adapter pattern (§6) is the contract when they land. |
