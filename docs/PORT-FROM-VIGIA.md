...
| #10-#15 narrative producers | NOT PORTED (infrastructure landed) | The narrative LAYER now exists (`src/narrative/`: post-seal narrator, Ollama + Anthropic backends, swap test, no-import boundary) — it is the surface the §6 adapters will feed. The producers themselves (stylometry, entropy, jitter, Grice score, LR) remain open; the non-sealable adapter pattern (§6) is the contract when they land. |
