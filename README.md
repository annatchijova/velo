...
```bash
npm install
npm test          # 115 engine tests, including adversarial ones
cd frontend && npx vitest run   # 116 more — 231 across both suites
...
| Layer | State |
|---|---|
| Deterministic engine + Daubert gate | **Working**, 115 tests |
| Test coverage across both suites | **231 green** — 115 engine (`npm test`) + 116 frontend (`vitest run` in `frontend/`). Counted by the runners, not estimated — `node scripts/count-tests.mjs` re-measures and fails if this line drifts |
