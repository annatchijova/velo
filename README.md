...
```bash
npm install
npm test          # 173 engine tests, including adversarial ones
cd frontend && npx vitest run   # 116 more — 289 across both suites
...
| Layer | State |
|---|---|
| Deterministic engine + Daubert gate | **Working**, 173 tests |
| Test coverage across both suites | **289 green** — 173 engine (`npm test`) + 116 frontend (`vitest run` in `frontend/`). Counted by the runners, not estimated — `node scripts/count-tests.mjs` re-measures and fails if this line drifts |
